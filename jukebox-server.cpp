 ...
 *
 * Authors: Christopher Eardensoh ceardensohn@sandiego.edu
 * 			Lucas Walbeck lwalbeck@sandiego.edu
 * 			Rory Abraham rabraham@sandiego.edu
 *
...
vector<struct music> find_mp3_files(const char *dir);
void event_loop(int epoll_fd, int server_socket, vector<struct music> song_list);
...
int main(int argc, char **argv) {
	 ...
	 */
    vector<struct music> song_list = find_mp3_files(argv[2]);
	...
	memset(&server_ev, 0, sizeof(struct epoll_event));
...
vector<struct music> find_mp3_files(const char *dir) {
	...
	for(fs::directory_iterator entry(dir); entry != fs::directory_iterator(); ++entry) {
		...
		fs::path mp3_path = entry->path().relative_path();
		...
		if (entry->path().extension() == ".mp3") {
			...
			if (fs::is_regular_file(info_file_path)) {
				...
				buffer << t.rdbuf();
				cout << "Info:\n" << buffer.str() << "\n";
				song.info = buffer.str();
			}

			music_list.push_back(song);
		}
	}
	cout << music_list.size() << '\n';
...
void setup_new_client(int server_socket, 
	...
	memset(&new_client_ev, 0, sizeof(struct epoll_event));
...
void event_loop(int epoll_fd, int server_socket, vector<struct music> song_list) {
    ...
    while (true) {
		...
		cout << "Number of epoll events: " << num_events << "\n";
